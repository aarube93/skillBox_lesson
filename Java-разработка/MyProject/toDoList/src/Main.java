...
public class Main {
    ...
    public static String lastCommandAdd = "";
    ...
    public static void commandLine() throws InterruptedException {
        ...
        while (true) {
            ...
            if (COMMAND_LIST()[8].equals(command)) {
                lastCommandAdd = "";
                StopWatch.start();
            }

            if (COMMAND_LIST()[7].equals(command)) {
                lastCommandAdd = "";

                if (getIndex(input) > 0 && getIndex(input) <= todoList.list.size()) {
                    todoList.add(getIndex(input), timeMachine.getCurrentTime());

                } else {

                    todoList.add(timeMachine.getCurrentTime());
                }
            }

            if (COMMAND_LIST()[6].equals(command)) {
                System.out.println(timeMachine.getCurrentTime());
            }
            ...
            if (COMMAND_LIST()[5].equals(command)) {

                calculator.start();
            }

            if (COMMAND_LIST()[2].equals(command) ||
                    lastCommandAdd.equals(COMMAND_LIST()[2])) {
...
//            i = true;
//        }
//        if (f && !lastCommandAdd.equals(COMMAND_LIST()[2])
//                && getArrayInput(input).length < 2) {
//
//            i = true;
//        }
...
//    }
    ...
    public static void getTruthOfCommand(String input) throws InterruptedException {
        ...
        boolean i = false;
        for (String item : COMMAND_LIST()) {
            if (item.equals(command) || item.equals(lastCommandAdd)) i = false;
        }
        if (command.equals(COMMAND_LIST()[2]) && !lastCommandAdd.equals(COMMAND_LIST()[2])
                && getArrayInput(input).length < 2) i = true;

        if (command.equals(COMMAND_LIST()[2]) && lastCommandAdd.equals(COMMAND_LIST()[2])
                && getArrayInput(input).length < 2) i = true;

        if (getArrayInput(input).length < 2
                && !lastCommandAdd.equals(COMMAND_LIST()[2])) i = true;

        if (i) System.out.println(DIALOG_LIST()[2]); commandLine();
    }
